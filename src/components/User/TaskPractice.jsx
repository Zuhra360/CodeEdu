  ...
  const handleSubmit = async () => {
    try {
      ...
      if (error) {
        ...
        toast.success('Question is complete!')
    ...
    <div className='w-11/12 md:h-[650px] sm:h-auto p-[10px] h-[700px] bg-[#EEEEEE] rounded-[10px] flex md:flex-row flex-col gap-[20px]'>
       {question ? (
         <div className='md:w-1/2 sm:w-full p-[10px] h-full text-md font-medium  bg-white flex flex-col justify-between rounded-[5px]'>
            ...
            <h1 className='text-2xl font-bold text-[#578E7E]'>{question.Title}</h1>
            <h1 className='text-md font-medium text-gray-700'>{question.Question}?</h1> 
            <h1 className='text-md font-medium text-gray-700 flex flex-col'><label className='text-[#123524] text-xl'>Test Case :</label>{question.Testcase}</h1>
            </div>
            <div className="flex flex-row justify-between">
            <button className="w-[120px] h-[30px] py-[5px] px-[10px] bg-[#638C6D] hover:bg-[#AAB99A]  text-white font-semibold text-lg rounded-[10px] flex items-center justify-center" onClick={handleSubmit} >Submit</button>
